...
! function(b) {
    ...
    t.prototype = {
        ...
        // Shoutout to https://stackoverflow.com/a/27406684/5437832
        genRan: function generateRandom(min, max, depth) {
            if(depth>10) { return 1; }
            var num = Math.floor(Math.random() * (max - min + 1)) + min;
            if (this.settings.slide != -1) {
                return (num === this.settings.slide) ? generateRandom(min, max, depth+1) : num;
            } else {
                return num;
            }
        },
        shuffle: function() {
            for (var t, s, i = this.total - 1; 0 < i; i--) {
                console.log( 'In for loop with i = %d', i);
                console.log( 'this.settings.slide = %d', this.settings.slide);
                if (this.settings.slide != -1) {
                    console.log( 'In the if section casue slide value is different than -1');
                    if (i == this.settings.slide) {
                        console.log( 'Skipping this loop cause i = our initial slide value.');
                        continue;
                    }
                    s = this.genRan(0,this.total-1,10),
